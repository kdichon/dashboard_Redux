...
const Ajouter = () => {
  ...
  const save = async () => {
    if (ask != "" && selectedCategory != "") {
      ...
      await addDataToCollection("TruthOrDare", data);
  ...
  const initData = async () => {
    const dataTemp = await getAll("CategoryDOT");
  ...
  return (
    <div className="flex flex-row gap-4">
      ...
      <select
        value={selectedDareOrTruth}
        onChange={handleDropDareOrTruth}
        className="select select-bordered w-full max-w-xs"
      >
      ...
      />
      <button onClick={save} className="btn btn-active btn-primary">
        Enregistrer
      </button>
    </div>
  );
